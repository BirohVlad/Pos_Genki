...
class FacturaService:
    ...
    def emitir_factura(self, db: Session, input_data: FacturarInput) -> FacturarResponse:
        ...
        for item in input_data.productos:
            ...
            if producto.tipo_iva == "2":
                ...
                # "0" = exento, "6" = no objeto — ambos van sin IVA
        ...
        factura = Factura(
            ...
            clave_acceso        = "",           # Se completará dentro de generar_xml
        ...
        try:
            # A. Generar XML (calcula y asigna la clave_acceso a la factura)
            xml_str, clave_acceso = sri_service.generar_xml(
                factura, factura.detalles, empresa, punto
            )
            db.commit()   # Persiste clave_acceso generada

            # B. Firmar XML con XAdES-BES
            ...
            # C. Enviar al SRI y hacer polling de autorización
            try:
                ...
                # Marcar estado según el tipo de error del SRI
            ...
            raise   # Re-propagar errores controlados tal cual
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # Método privado: bloqueo pesimista del secuencial
    # ──────────────────────────────────────────────────────────────────────────
    ...
    def _obtener_y_bloquear_secuencial(
        ...
        """
        Bloquea la fila del PuntoEmision con SELECT FOR UPDATE (PostgreSQL) para
        garantizar que en entornos con múltiples workers no se generen dos facturas
        con el mismo secuencial.

        En SQLite (desarrollo local) se omite el bloqueo porque no está soportado.
        """
        try:
            ...
            if not is_sqlite:
                query = query.with_for_update()  # Solo en PostgreSQL
            ...
            if not punto:
                raise HTTPException(
                    ...
                    detail=(
                        ...
                        f"'{punto_emision}' no configurado en la base de datos."
                    )
                )
            ...
            db.commit()   # Commit inmediato para liberar el bloqueo de fila
